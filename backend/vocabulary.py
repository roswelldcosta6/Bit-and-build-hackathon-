...
Includes emergency, healthcare, banking, educational, and daily conversational vocabulary.
"""
...
# Maps common English and Hindi words/phrases to standardized ISL glosses
...
def get_all_vocabulary() -> List[Dict[str, Any]]:
    """Returns the complete master list of vocabulary items."""
    return ISL_VOCABULARY


def lookup_gloss(word: str) -> Optional[str]:
    """
    Look up a single word (English or Hindi) in the synonym map.
    Returns normalized ISL gloss or None if not directly matched.
    """
...
def get_sign_metadata(gloss: str) -> Optional[Dict[str, Any]]:
    """Retrieve metadata (clip_id, translations, category) for an ISL gloss."""
